...
export function renderer_start(cssSelector) {
    var draw = SVG(cssSelector).size(window.innerWidth, window.innerHeight / 2);
    var fnt = new Heebo();
    var disp = fnt.drawNumbers(draw,(window.innerWidth/4), 10,'20:59:59');
    console.log(disp);
    setInterval(function() { updateClock(fnt, draw, disp); }, 1000);
}

function updateClock(font, draw, disp) {
    font.transitionNumbers(draw, (window.innerWidth/4), 10, disp, curTimeString());
}
